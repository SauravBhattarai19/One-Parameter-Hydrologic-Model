...
config.py
=========
OpmConfig — the single configuration object for the VSA-OPM model.
...
OpmConfig is the canonical way to parameterise a run from the Python API, the
CLI (via a YAML/JSON file) or the QGIS plugin UI.

Usage
-----
    from vsa_opm import OpmConfig

    cfg = OpmConfig(DEM_PATH="/path/to/dem.tif", BACKEND="gpu")
    cfg.OUTPUT_DIR = "/path/to/results"
    cfg.update_output_paths()

    # or from a config file (YAML / JSON / legacy python module):
    cfg = OpmConfig.from_file("config.yaml")
"""
...
def _data_path(filename):
    """Absolute path of a lookup file shipped inside the package (vsa_opm/data)."""
...
class OpmConfig:
        ...
        for key, value in kwargs.items():
            if not hasattr(self, key):
                raise AttributeError(
                    ...
                    f"Check vsa_opm/config.py for valid parameter names."
                )
    ...
    def from_dict(cls, data):
        """
        Build an OpmConfig from a plain dict of {PARAMETER: value}.
        ...
        if unknown:
            raise AttributeError(
                ...
                + ".  Check vsa_opm/config.py for valid names."
            )
        cfg = cls()
    ...
    def from_file(cls, path):
        ...
        if ext == ".py":
            ...
            spec = importlib.util.spec_from_file_location("_vsa_opm_user_config", path)
    ...
    def to_dict(self):
        ...
        keys = [k for k in vars(OpmConfig)
